...
function cache(options) {
  ...
  function get(name, cb) {
    if (ram && !!memoryCache[name]) {
      ...
      const newEntryData = JSON.parse(entry.data);

      if (newEntryData.body && newEntryData.body.type === "Buffer") {
        newEntryData.body = Buffer.from(newEntryData.body.data);
      }

      newEntryData.rawBody.map(body => {
        if (body.type === "Buffer") {
